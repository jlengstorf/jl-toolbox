...
test('resolveKcdScripts resolves to src/index.js when in the jl-toolbox package', () => {
  mockPkg({ pkg: { name: 'jl-toolbox' } });
  expect(require('../utils').resolveKcdScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  );
});

test('resolveKcdScripts resolves to jl-toolbox if not in the jl-toolbox package', () => {
  mockPkg({ pkg: { name: 'not-jl-toolbox' } });
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'));
  expect(require('../utils').resolveKcdScripts()).toBe('jl-toolbox');
});
